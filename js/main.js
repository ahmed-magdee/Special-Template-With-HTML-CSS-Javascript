...
let spanPregress = document.querySelectorAll(
...
allLinks.onclick = function (e) {
  // Stop Propagation Of links Header
  e.stopPropagation();
};
gearIcon.onclick = function () {
  // Gear Icon To Close And Open The Settings
  this.classList.toggle("fa-spin");
  settingsMenu.classList.toggle("open");
};
document.addEventListener("click", (e) => {
  // To Close The Menu Settings
...
settingsMenu.onclick = function (e) {
  // Stop The Propagation Of Settings Menu
  e.stopPropagation();
};
if (colorLocalStorage !== "") {
  // Change The Color By LocalStorage
...
settingsLinksUl.forEach((li) => {
  // Send Colors To LocalStorage And Change Style The Page
  li.onclick = function () {
    removeClass(settingsLinksUl);
...
function removeClass(elements) {
  // Function To Remove On All Elements
...
imgsSettings.forEach((img) => {
  // Imgs Settings Section
  if (checkBG === true) {
    document.querySelector(".yes").classList.add("active");
  }
...
spanPregress.forEach((span) => {
  // The Number Of Width The Spans Progress
...
function sectionOpacityOne(section) {
  // Sections To Opacity 1
